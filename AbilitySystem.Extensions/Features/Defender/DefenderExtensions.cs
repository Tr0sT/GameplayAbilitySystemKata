...
namespace AbilitySystem
{
    public static class DefenderExtensions
    {
        ...
        public static void AddDefenderStatusEffect(this IUnit unit, ICommandQueue commandQueue, ICombatEventsContext combatEventsContext)
        {
            if (!unit.IsBully())
            {
                unit.GetCombatFeature<IStatusEffectsHolder>().AddStatusEffect(
                    new Defender(unit, commandQueue, combatEventsContext));
            }
        }
    }
}